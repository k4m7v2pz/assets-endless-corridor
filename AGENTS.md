...
- 人类偏好变更（如"素材目录命名方式"）→ 在"附录"里记设计原则
- 新的自动行为约定 → 在本文件里记成规则
- 素材授权或来源变化 → 在"附录"里记素材约定
...
- **目录**：`sprites/`、`tiles/`（美术）；`ambience/`、`audio/`、`sounds/`（音效与背景音）；`fonts/`（字体）；`design/`（设计文件，含 FL Studio 工程）。
- **命名**：面向开发的命名一律用英文 snake_case；Finder/UI 里可中文，但进仓库的文件名用英文。
- **源工程保留**：`design/fl_studio_projects/` 里的 `.flp` 工程是原创性铁证，**不得删除**。

### B. 授权与署名

- 本仓库素材按 **Mulan OWL BY-PL v1** 授权，使用、修改、传播须保留署名，作者 `k4m7v2pz`。
- **例外**：`audio/music/suno_ai/` 内音乐由 Suno AI 免费计划生成，**仅限个人非商业使用**，不适用仓库统一授权——本项目永不盈利，故不受影响，但标注不得删除。
